...
1. **EPS**: 연결 연간 EPS 막대 그래프 + YoY 증가율(%) 꺾은선 (이중 Y축)
...
6. **ROE**: 연간 자기자본이익률(%) 8개년(추정치 3개년 포함) 막대 그래프
7. **Free Cash Flow**: FnGuide 고유 양식을 복제한 3대 현금흐름(영업활동, 투자활동, 재무활동) 그룹 막대 그래프 (연간/분기 전환 지원)
8. **컨센서스 시계열 추이(Q)**: 분기 기준 컨센서스(최고, 최저, 평균) 꺾은선 그래프 (매출액, 영업이익, 당기순이익, EPS, PER, PER(Fwd,12M) 지표 선택 지원)
9. **컨센서스 시계열 추이(Y)**: 연간 기준 컨센서스(최고, 최저, 평균) 꺾은선 그래프 (지표 선택 지원)
10. **적정주가 추이**: 증권사별 목표주가 리포트 시계열 + Consensus 평균 목표주가 수평 기준선
